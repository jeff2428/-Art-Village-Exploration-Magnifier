...
from views import shell_view as sv
...
async def _run_app(page: ft.Page) -> None:
    ...
    start_button = wv.build_start_button(
        on_click=lambda _e: create_bg_task(lifecycle.start_exploration(shell, welcome_screen, start_button, camera))
    )
    ...
    mode = ft.Container(content=sv.build_mode_selector(selected_mode, lambda v: create_bg_task(lifecycle.switch_mode(v))))
